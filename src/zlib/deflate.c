 ...
 */
#define CLEAR_HASH(s) \
    zmemzero(s->head, (unsigned)(s->hash_size)*sizeof(*s->head));
 ...
 */
static unsigned read_buf(z_streamp strm, uint8_t *buf, uint32_t size) {
...
static void fill_window(deflate_state *s) {
    ...
    do {
        ...
        if (s->strstart >= wsize + MAX_DIST(s)) {
            ...
            s->match_start -= wsize; /* we now have strstart >= MAX_DIST */
            s->strstart    -= wsize;
...
int deflateInit2_(z_streamp strm, int level, int method,
    ...
    s->pending_buf = (uint8_t *) ZALLOC(strm, s->lit_bufsize, LIT_BUFS);
...
static int deflate_ (z_streamp strm, int flush, int put) {
 ...
 */
int deflateCopy(z_stream* dest, z_stream* source, unsigned char alloc) {
  deflate_state *ds;
  deflate_state *ss;
  ...
  if(alloc) {
    zmemcpy(dest, (voidpf)source, sizeof(z_stream));
  ...
  if(alloc) {
    ...
    ds->pending_buf = (uint8_t *) ZALLOC(dest, ds->lit_bufsize, LIT_BUFS);
  }
...
ALWAYS_INLINE inline static uint32_t longest_match(deflate_state *s, IPos cur_match) {
    ...
    do {
        ...
        if (best_len < MIN_MATCH) {
          ...
          for (;;) {
            IPos cur_match2 = cur_match - OFF;
              if (*(uint64_t*)(match_base3 + cur_match2) == scan_end64 &&
                  *(uint64_t*)(win + cur_match2) == scan_start64) break;
              NEXT_CHAIN;
          }
          cur_match -= OFF;
        } else {
            ...
            for (;;) {
            IPos cur_match2 = cur_match - OFF;
                if (*(unsigned*)(match_base2 + cur_match2) == scan_end32 &&
                    *(unsigned*)(win + cur_match2) == scan_start32) break;
                NEXT_CHAIN;
            }
            cur_match -= OFF;
        }
      ...
      do {
        ...
        if (len > best_len) {
            ...
            if (s->good_match == MAX_MATCH + 1) {cur_match = prev[cur_match & wmask]; continue;}
            IPos    pos, next_pos;
            limit -= OFF;
            OFF = 0;
